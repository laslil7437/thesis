...
def parse_args():
    parser = argparse.ArgumentParser(description="Fine-tune GPT-2 on TinyStories dataset.")
...
# modified below to evaluate blimp, instead of   
def evaluate_model(model: nn.Module, dataset: data.Dataset, batch_size: int) -> dict:
    results = {}
    for task, dataset in datasets.items():
        ...
        results[task] = total_loss.item() / total_num
        print(f"Loss: {results[task]}")
    return results
...
def main():
    ...
    train_dataset = get_tinystories_dataset(split="train")
    model = train(
    ...
    eval_train_dataset = get_tinystories_dataset(split="eval_train")
