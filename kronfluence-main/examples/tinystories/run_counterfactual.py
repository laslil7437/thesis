...
import sys
sys.path.append("/Users/lilylassiter/Desktop/kronfluence-main/examples/tinystories")

from pipeline import get_tinystories_datset
...
def main():
    ...
    train_dataset = get_tinystories_datset(split="train")
    # You might need to change the path.
    identity_scores = Analyzer.load_file("influence_results/tinystories/scores_identity/pairwise_scores.safetensors")[
        "all_modules"
    ][:50].sum(dim=0)
    ekfac_scores = Analyzer.load_file("influence_results/tinystories/scores_ekfac/pairwise_scores.safetensors")[
        "all_modules"
    ][:50].sum(dim=0)
    ...
    valid_dataset = get_tinystories_datset(split="valid", indices=list(range(50)))

    def train_and_evaluate(indices) -> float:
        model = train(
            dataset=get_tinystories_datset(split="train", indices=indices),
            batch_size=8,
            num_train_epochs=3,
            learning_rate=3e-05,
            weight_decay=0.01,
