...
from pipeline import get_tiny_stories_dataset
...
def main():
    ...
    scores = Analyzer.load_file("influence_results/tinystories/scores_ekfac_half_per_token/pairwise_scores.safetensors")[
        "all_modules"
    ...
    eval_idx = 5
    train_dataset = get_tiny_stories_dataset(
        split="eval_train",
    )
    eval_dataset = get_tiny_stories_dataset(
        split="valid",
    )
